...
`MockEnrichmentAgent` is a test-only implementation of `IEnrichmentAgent` that returns a fixed, hardcoded `EnrichedTicket` without making any external calls. It is used when `--mock` mode is active to enable full pipeline execution without Azure OpenAI credentials.
