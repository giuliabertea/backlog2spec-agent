...
`MockAdoClient` is a test-only implementation of `IAdoClient` that returns a fixed, hardcoded `WorkItemDto` without making any external calls. It is used when `--mock` mode is active to enable full pipeline execution without Azure DevOps credentials.
