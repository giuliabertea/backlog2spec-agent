...
`LlmKeywordExtractor` is responsible for extracting relevant search keywords from a work item using an LLM call, enabling `CodebaseContextAgent` to find related source files more accurately than simple stop-word splitting. It implements `IKeywordExtractor` and falls back to `StopwordKeywordExtractor` on any LLM failure.
