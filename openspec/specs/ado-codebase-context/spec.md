# ado-codebase-context

## Purpose

`CodebaseContextAgent` is responsible for fetching relevant source files from an Azure DevOps Git repository based on keyword relevance to a work item. It provides code snippets to the enrichment pipeline so that the LLM can ground component names and context in the actual codebase rather than guessing.
