# copilot-optimised-spec-format

## Purpose

Defines the structure and field contract for `GeneratedSpec` — the output model produced by `SpecGeneratorAgent`. The format is optimised for Copilot consumption: fields are named and scoped to match how a developer (or an AI coding assistant) would act on them.
