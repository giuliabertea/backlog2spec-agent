...
`MockCodebaseContextAgent` is a test-double implementation of `ICodebaseContextAgent` that returns a hardcoded list of `CodeFileDto` objects without making any HTTP calls. It is used when the CLI is invoked with `--mock` to allow end-to-end pipeline testing without ADO credentials or repository access.
